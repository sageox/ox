...
func TestRenderStripSVG_themes(t *testing.T) {
	...
	mustContain(t, ls, "GUIDED BY SAGEOX", `aria-label="Guided by SageOx · 2 prior art · 1 collision"`)
...
func TestStripContentName_contentAddressed(t *testing.T) {
	a := stripContentName(Signals{PriorArt: 2, Collisions: 1, ExpertRoutes: 0, Material: true})
	b := stripContentName(Signals{PriorArt: 2, Collisions: 1, ExpertRoutes: 0, Material: true})
	c := stripContentName(Signals{PriorArt: 3, Collisions: 1, ExpertRoutes: 0, Material: true})
	if a != b {
	...
	if a != "enrich-2-1-0" {
		t.Errorf("name = %q, want enrich-2-1-0", a)
	}
}
...
func TestUploadStrip_success(t *testing.T) {
	...
	mustContain(t, urls.Light, "enrich-2-1-0-light.svg")
	mustContain(t, urls.Dark, "enrich-2-1-0-dark.svg")
	if len(f.calls) != 2 {
