...
func selectDesiredSkills(version string, desired DesiredSkills) ([]skills.Skill, error) {
	...
	return markUnprefixedOwnership(selected), nil
}

// markUnprefixedOwnership gives catalog skills that cannot prove ownership by
// NAME a durable, in-band signal that they are ox's — so ox can still recognize
// its own materialized copy after the machine-local state cache is gone.
//
// The problem it solves. Per-file ownership digests live in
// .sageox/cache/skills-state.json, and that file is correctly disposable:
// readLocalState resolves both "missing" and "corrupt" to EMPTY state rather
// than wedging the repository. A PREFIXED skill shrugs that off, because its
// name is the proof — IsReclaimableName answers before anything is read. An
// unprefixed catalog skill had no second signal at all, so a lost cache meant ox
// could no longer distinguish its own post-cutoff from a stranger's: conflict
// forever, and no route to ever ship that customer an update again.
//
// Why a stamp rather than comparing against the bytes ox ships today. Content
// equality recognizes only a copy written from the CURRENT catalog, and the case
// that actually matters is the opposite: the cache is gone AND the catalog has
// moved on, which is exactly when an update is owed. A stamp hashes the body it
// sits above, so it keeps verifying whichever release wrote it — and
// validLegacyStamp, the check that consumes it, already exists.
//
// Why here rather than baked into extensions/skills/<name>/SKILL.md. A stamp in
// the source would hash a body it lives inside, so every content edit would need
// its hash regenerated by a build step, and a stale hash fails SILENTLY — it
// stops proving ownership and nothing says so. Computing it as the skill is
// selected makes it correct by construction and keeps the authored source clean.
// Because this runs inside the catalog projection, the stamped bytes are what
...
func markUnprefixedOwnership(selected []skills.Skill) []skills.Skill {
	for i := range selected {
		...
		for j := range skill.Files {
			if skill.Files[j].Path != skills.SkillFileName {
				continue
			}
			stamped := stampBelowFrontmatter(skill.Files[j].Content, skill.Version)
			skill.Files[j].Content = stamped
			// Skill.Content is a second copy of SKILL.md kept for simple adapter
			// consumers. Letting the two diverge would mean an adapter installs
			// unstamped bytes that ox then refuses to recognize as its own.
			skill.Content = stamped
		}
	}
	return selected
}

// stampBelowFrontmatter inserts the ownership stamp AFTER any leading YAML
// frontmatter rather than at the top of the file.
//
// Placement is not cosmetic. agentx.StampedContent prepends, and a stamp above
// the frontmatter pushes `---` off line 1, which stops an agent harness from
// parsing the manifest at all: the skill's description — its entire
// auto-selection budget — is replaced by the stamp comment, and a skill nothing
// fires on does not exist. Legacy ox stamped command files that way and those
// files are still visibly broken. Frontmatter first, stamp, then body is the
// shape the rest of ox already reads (see internal/adapterstamp).
func stampBelowFrontmatter(content []byte, version string) []byte {
	head := frontmatterEnd(content)
	out := make([]byte, 0, len(content)+64)
	out = append(out, content[:head]...)
	return append(out, agentx.StampedContent(content[head:], version, stampPrefix)...)
}

// frontmatterEnd returns the offset just past a leading `---` fenced block, or 0
// when the content does not open with one. An UNTERMINATED block also returns 0:
// that is a malformed manifest the catalog's own Validate rejects before it can
// be selected, and guessing an end offset would split the file mid-metadata.
func frontmatterEnd(content []byte) int {
	...
	for offset < len(content) {
		...
		if end < 0 {
			if bytes.Equal(bytes.TrimRight(content[offset:], " \t"), []byte("---")) {
				return len(content)
			}
			return 0
		}
