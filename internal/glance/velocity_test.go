...
func TestConflictVelocity_DailyBuckets(t *testing.T) {
	...
	points := ConflictVelocity(sessions, t0, t0.Add(3*day), day, day)
	...
	assert.Equal(t, 3, points[0].Sessions, "day 1 sessions")
...
func TestConflictVelocity_EmptyWindow(t *testing.T) {
	...
	assert.Equal(t, 1, points[0].Sessions)
	assert.Equal(t, 0, points[1].Sessions, "day 2 should be empty")
	assert.Equal(t, 0, points[2].Sessions, "day 3 should be empty")
}

func TestIsEscalating_True(t *testing.T) {
	points := []VelocityPoint{
		{Conflicts: 1, Sessions: 4},
		{Conflicts: 3, Sessions: 6},
		{Conflicts: 8, Sessions: 10},
	}
...
func TestIsEscalating_SkipsEmptyWindows(t *testing.T) {
	points := []VelocityPoint{
		{Conflicts: 1, Sessions: 4},
		{Conflicts: 0, Sessions: 0}, // empty window — skipped
		{Conflicts: 3, Sessions: 6},
		{Conflicts: 8, Sessions: 10},
	}
