...
// shape. It deliberately includes origin, delivery, and commit so a successful
...
func WriteText(w io.Writer, report Report) error {
	...
	for _, outcome := range report.Outcomes {
		origin := string(outcome.Origin.Kind)
		if outcome.Origin.Kind == OriginPack {
			origin = "pack " + outcome.Origin.Pack
			if outcome.Origin.PackVersion != "" {
				origin += "@" + outcome.Origin.PackVersion
			}
		}
		delivery := ""
		if outcome.Delivery != "" {
		...
		if _, err := fmt.Fprintf(w, "  %s/%s: %s%s (%s)%s\n",
			outcome.Kind, outcome.Name, outcome.State, delivery, origin, detail); err != nil {
			return err
		}
	}
	return nil
