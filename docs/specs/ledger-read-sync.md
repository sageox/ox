...
An existing `path` that is not a checkout is adopted when it holds nothing but ox's own cache, `.sageox/cache` — typically a code index built before the first refresh — or nothing at all. It is cloned exactly as an absent `path` is, through the stage, and left untouched while the clone and hydration run, so an attempt that fails or is interrupted before publication leaves it as it was. At publication its cache is copied into the verified stage, by the same restore the background daemon's reclone uses, and `path` is replaced only if it still holds nothing but that cache. The copy is local and is not interrupted by `--timeout`, SIGINT, or SIGTERM; its duration grows with the size of the cache. Anything else at `path` — a file, a symlink, or a directory holding anything other than ox's cache — is refused as `path_occupied` and left exactly as it is, as is a `path` whose parent is not a directory or that cannot be inspected.
