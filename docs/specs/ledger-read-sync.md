...
| `resumable` | `true` when the checkout is still unpublished and this attempt left a staged clone that the next attempt continues from instead of cloning from empty; omitted otherwise. See [Remote evidence and local recovery](#remote-evidence-and-local-recovery). |
