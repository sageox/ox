...
- **Budget:** `--timeout` is a positive Go duration, default `30m`. One context bounds discovery, waiting for the checkout lock, Git subprocesses, materialization, and hydration. The default is sized for a cold clone of a large ledger, whose cost is dominated by transferring every LFS object it covers; a warm refresh finishes in seconds. Set your own budget rather than relying on this one, and give a background refresh and an individual tool call separate budgets. SIGINT and SIGTERM cancel the operation. Runtime cancellation returns `interrupted` when the process can render a result; SIGKILL cannot produce JSON.
...
| `resumable` | `true` when the checkout is still unpublished and this attempt left a staged clone bound to this repository, endpoint, and read URL — one the next attempt continues from, after proving it, instead of cloning from empty. Omitted otherwise. See [Remote evidence and local recovery](#remote-evidence-and-local-recovery). |
...
Failure categories are `invalid_arguments`, `denied`, `unavailable`, `missing_ledger`, `interrupted`, `dirty`, `missing_hydration`, `incomplete_history`, `incomplete_coverage`, `identity_mismatch`, `path_occupied`, and `git_failed`. Authentication failures, including a missing selected TAT, use `denied`. A deadline or canceled lock wait uses `interrupted`. `unavailable` also covers discovery transport failure and an unusable backend response. `path_occupied` means ox cannot claim `path` and will not remove what holds it (see [Remote evidence and local recovery](#remote-evidence-and-local-recovery)); unlike `interrupted`, no retry succeeds until the path is cleared. Consumers must tolerate additional failure categories in future releases.
...
An existing `path` that is not a checkout is adopted when it holds nothing but ox's own cache, `.sageox/cache` — typically a code index built before the first refresh — every file of which ox can read, or holds nothing at all. It is cloned through the stage like an absent `path` and left untouched until publication, so a failed or interrupted attempt leaves it as it was. At publication its cache is copied into the verified stage and `path` is checked again: only the cache is removed recursively, and the directories above it only once empty, so anything written into `path` outside the cache during the attempt is refused rather than removed. A write into the cache after the copy is lost, including an append to a session recording in progress. The copy is local and is not interrupted by `--timeout`, SIGINT, or SIGTERM; its duration grows with the size of the cache. Anything else at `path` — a file, a symlink, or a directory holding anything other than a readable ox cache — is refused as `path_occupied` and left exactly as it is, as is a `path` whose parent is not a directory or that cannot be inspected. At publication, such content or a `path` ox may not remove is `path_occupied` too, and any other failure to clear `path` is `interrupted`; either way the verified stage is kept, so the next attempt publishes without cloning again.

A failed attempt that leaves a stage bound to this identity reports `resumable: true`; `ready` stays `false`, and `error_class` and `error_detail` are those of the failure. Its `coverage` and `hydration` describe the stage as the attempt last counted it — the files materialized, and how many pointer files are still stubs — so an attempt that got most of the way is distinguishable from one that did nothing. Verification that stops before it counts — HEAD, its history, or a file fails it, or the budget runs out — leaves the counts hydration took. Like `ready`, it is a statement about the state the attempt left, not a guarantee about later changes: the next attempt still proves the stage under the lock, as described above, before continuing from it, and a stage whose worktree was changed in the meantime is replaced by a fresh clone. Whether the stage's worktree still matches its HEAD is not re-proved when the failure is reported — that hashes every covered file, and an attempt whose budget expired cannot do it. An attempt that failed before counting, such as a resumed attempt whose fetch fails, reports `hydration.state: "unknown"` with zero counts even when `resumable` is `true`. An attempt that failed before its stage had a worktree reports neither, and the next attempt clones from empty.
...
A batch's granted objects are downloaded concurrently, at most 8 at a time, and fewer while the server refuses them. The read route bounds how many transfers it serves at once and refuses each one over that bound on arrival with a 503 or 429 rather than queueing it; the client does not know the bound's number and does not need to. Each refusal lowers the number of requests the client keeps at the server by one, never below one, and each run of successes as long as that number raises it by one, never above 8 — so the client settles at whatever the server serves and follows the bound when it changes. A refused download gives up its place while it waits to retry, and the number is not raised while any refused download is still waiting: its retry is what shows whether the server has room again. Concurrency changes only how long the transfer takes: each object is still verified by OID and by size and still committed durably, and the object `error_detail` names is the first one in batch order rather than the first to fail in wall-clock time. A failure that stops hydration also stops the transfers running beside it, so a grant the server stops honoring mid-batch does not keep delivering objects behind the refusal; the transfers it cancels leave their files as stubs and report nothing of their own.

A failed batch request or object download is retried. Only a failure a repeat could get past is retried — a transport failure, or the server's own 5xx or 429. A canceled operation, an unusable read credential, any other status including 401/403 and 404, content that does not hash to the object's identity, and a batch response the server delivered in full but that cannot be used are settled answers and are not retried. A response cut short by the transport is not one of those — truncation fails while the body is read, before anything tries to decode it. A retry never relaxes a verification: a partially transferred object is discarded rather than continued, and the size check still runs on what finally arrives.

Retrying one request is bounded by attempts and by time. A failure other than a refusal is retried at most twice more, after a 200 ms pause that doubles each time. A refusal — a 503 or 429 — is not counted against those attempts, because a server shedding load refuses in about a millisecond and three lost races for its bound say nothing about the object; a refusal is retried after the `Retry-After` the response carries, in either delay-seconds or HTTP-date form, and after the pause above when it carries none or an unparseable one. Every wait is clamped to the request's retry window, one thirtieth of what remains of `--timeout` (a minute at the start of the default 30m), so a hostile `Retry-After` costs at most one window, and no one request can keep retrying past its window while the objects behind it wait. A download still refused when its window closes is skipped like any other object the server would not serve — `download_refused`, with the server's status — and a batch request still refused loses that batch's grants; neither spends the rest of the budget or turns the sync into `interrupted`.
