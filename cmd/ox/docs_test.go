...
func TestPrepareDocsCommandTreeExcludesExperimentalCommands(t *testing.T) {
	...
	bulletin := &cobra.Command{Use: "bulletin"}
	...
	root.AddCommand(attest, bulletin, scout, memory, carts, cartAnalyze, completion)

	prepareDocsCommandTree(root)

	for _, command := range []*cobra.Command{attest, bulletin, scout, memory, completion} {
