...
func TestBuildTemplateData_BasicSession(t *testing.T) {
	...
	data := sessionhtml.BuildTemplateData(st, nil)

	// title derives from filename when no AgentType in meta
	assert.Equal(t, "test-session", data.Title)
...
// mapEntryType and mapRoleToType tests moved to internal/session/html package
// where those functions now live (unexported). See generator_test.go.
...
func TestBuildTemplateData_FallbackDuration(t *testing.T) {
	...
	require.NotNil(t, data.Metadata)
	// with zero CreatedAt, StartedAt remains zero
