...
Knowledge Bubbles are re-scoped to what they're actually for, plans get a lifecycle you can track end-to-end with a genuinely better reviewing surface, and a session's link now resolves the moment it starts instead of when it ends.

### Removed

- **`ox kb path`, `ox kb hydrate`, and `ox kb config` are gone, with no replacement.** These treated a knowledge bubble as a workspace you could browse into, pull large files from, or configure directly — bubbles were never meant to work that way. Scripts or aliases depending on any of these now fail with a normal "unknown command" error.
- **`ox import --kb` is gone.** A knowledge bubble is curated, read-only team knowledge; a raw recording never belonged in one. `ox import --team`, URL import, and `--status`/`--list` are unchanged.

### New

- **Your team's published knowledge bubbles are now discovered and mounted automatically.** `ox kb list` and `ox kb describe <slug>` (renamed from `ox kb show`, which still works as an alias) show each bubble's real description, topics, steering guidance, and last activity, `ox status` shows a card per bubble with its local path and sync status instead of a bare count, and `ox agent prime` explains what's available so your AI coworker can pull in curated team knowledge without being pointed at it by hand. Nothing to turn on — if your team has published bubbles, they show up. Personal bubbles aren't available yet.
- **Track a plan's status from idea to done** — `ox plan work`, `approve`, `realized`, `abandon`, and `supersede` record what's happening to a plan as it moves through its lifecycle, `ox plan list` and `ox plan show` display that status alongside which sessions touched it, and every plan gets a stable ID the moment it's saved so its history follows it across revisions.
- **A session's link resolves the moment it starts, not when it ends.** An AI coworker writes its session link into the pull request it opens, but until now that link pointed at nothing until the session finished — a correctly wired setup was indistinguishable from a broken one right when you were most likely to check. ox now publishes a placeholder a couple of turns in (no conversation content whatsoever, just the session id, which agent is running, and a live turn counter), replaced by the real recording when the session ends; `ox session status` shows its state so you can tell a stuck link from a slow one. Aborting a session removes the placeholder too, and `ox doctor` retracts any left stranded by a crash. Turn it off with `ox config set session.draft off`.
- **`ox invite` brings teammates aboard without leaving the terminal.** `ox invite alice@acme.com,bob@acme.com` invites several people at once (`--team`, `--role`), with each address reported on its own line so one typo never hides everyone else's outcome. `ox invite --list` shows what's still outstanding, and `--cancel <invite id>` withdraws one.
- **Goose now has Silver-tier AI coworker support** — sessions record to the Ledger and team context primes automatically, the same as Claude Code or Codex. One caveat: Goose has no compaction event, so team context primed at session start is lost when Goose compacts, and ox can't work around that.
- **`ox viz` gives every AI coworker an editorial diagram catalog for any artifact** — ask `ox viz suggest "<what needs explaining>"` for a deterministic local match, pull an accessible recipe with `ox viz <id>`, render existing data-driven fragments, or lint authored SVG for portability and visual craft. Eight patterns ship in SageOx's light/dark token system, usable in docs, PRs, reports, and design notes — not just plans, which still work through the same catalog underneath.
- **`ox login` now labels each session so you can tell your machines apart.** Logging in attaches a short label like `ryan@laptop` to that session, shown next to it on your account's security page — so if you're signed in from a laptop, a desktop, and a CI box, you can tell which one to revoke. Automatic, no setup; opt out with `SAGEOX_NO_DEVICE_LABEL=1`.
...
- **A rendered plan is now a genuinely better reviewing surface.** `ox plan render` and `ox plan review` show the whole plan as one continuous document with a sticky, color-coded jump bar instead of tabs that hid every section but the one you were on, and they now recognize the shapes real plans are actually written in — tracks, gates, shipped/blocked status, risk — turning them into swimlanes and severity call-outs instead of flat prose. Any HTML files a plan links to are reliably bundled alongside it as inspectable companion cards, and the result looks and behaves the same no matter which coding agent wrote the plan.
- **Four new plan visualizations, so a rendered plan reads like it was designed.** A pull-quote for the one sentence the plan turns on, an honest progress pair that shows shipped beside not-built-yet instead of claiming delivery in prose, a risk register that puts every risk in one scannable row and hides the mechanism behind a click, and the SageOx wordmark lockup — all keyboard-operable and screen-reader-labeled.
...
- **`ox kb describe` no longer renders a bubble's text unsanitized.** A bubble's description or curator guidance now displays safely even if the content were ever compromised — hidden terminal escape codes can no longer repaint your screen, forge a fake row, or hijack your clipboard.

### Fixed

- **Session recording is now correct and complete across every supported coding agent.** Gemini, Codex, OpenCode, Goose, Droid, Amp, and Pi could each silently fail to record, record the wrong content, or (Gemini) even fail to start — ox reported a healthy recording the whole time regardless. All seven are now verified against real transcripts. Separately, a session the AI marks as "nothing worth summarizing" no longer gets stuck retrying forever; it finalizes cleanly on the first attempt.
- **Team contexts and ledgers are fully independent of knowledge bubbles again.** `ox teams` is no longer deprecated — ignore any earlier guidance to switch to `ox kb list --type=team`. A knowledge bubble no longer has any say in whether your sessions record, and `ox doctor` no longer flags a repo as an orphan while waiting on a bubble binding that was never coming.
- **A mounted knowledge bubble now reliably matches what's actually published.** Bubbles for a non-default endpoint used to save into your default/production directory instead of their own, risking bubbles from different endpoints colliding in one namespace; separately, a bubble could report itself fully synced while most of its files silently never reached disk, with no error either time. Both repair automatically on the next sync — nothing you need to do.
- **Ledger sync no longer gets permanently stuck.** A rebase used to abort back to square one the moment more than one commit conflicted, a stale lock could do the same, a detached HEAD could silently strand weeks of sessions with no repair offered, and a signing passphrase prompt could hang sync on two paths that never inherited an earlier fix — real cases got stuck for days to weeks. All four are now caught and either self-heal or get a rescue branch before anything can be lost.
- **A recording's link and content stay correct through retries and background sync.** A retried upload could mint a second permanent link for the same session, so references in commits, pull requests, and plans stopped resolving; separately, a background sync error could — very rarely — silently replace a recording's saved content with an empty file, with no error and no doctor check to catch it. Both are fixed.
- **Four `ox doctor` checks that were registered but silently never ran now run** — tracked `.rej` artifacts, stray local-only cache files, sparse-checkout drift, and legacy session IDs are caught again.
- **Plans you save are attributed correctly, and say so honestly when they can't be.** A single early return used to throw away the author, repo, and session link together whenever an agent id wasn't detected, so most saved plans landed anonymous and session-less with no way to recover the link later. Attribution now resolves independently through your OAuth identity, git config, or OS username, and a plan that truly can't be attributed is left blank instead of signed "Anonymous." `ox plan lint` also stops reporting a clean bill of health on checks it never ran, and saving a plan can no longer silently revive a session you'd already stopped or overwrite what it had recorded since.
- **Prior art on a plan is now actually relevant, and diagrams render correctly.** Relevance scoring was biased toward long documents that merely mentioned your search terms, so unrelated sessions could surface as 95%-confidence prior art; it now measures how much a document is genuinely about your terms. Diagrams also no longer clip their labels mid-word — they were being sized before the page's font finished loading.
- **`ox init` reliably stages everything it writes, and stops touching your root `.gitignore`.** A single bad file path used to silently break staging for the whole batch — hooks, skills, rules, instruction files, for any agent — so a fresh init could look complete while leaving new files untracked until you found and staged them yourself; that's fixed for every supported agent. Its own knowledge-bubble ignore rule also no longer gets written into your project's root `.gitignore`; already-affected repos clean up the next time you run `ox init` or `ox doctor --fix`.
- **`ox daemon start --foreground` now actually takes over instead of quietly doing nothing.** If another daemon was already running, this used to print "Daemon is already running" and exit as if it had succeeded, even though nothing was left running in the foreground — so a container or supervisor watching that process would restart it forever without ever getting a working daemon. It now stops the existing daemon and takes its place.
