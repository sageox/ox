...
- **Your AI coworker gets your whole team context, not a preview of it** — Claude Code keeps only the first couple of thousand characters of a hook's output and writes the rest to a file, so `ox agent prime` — which runs well past that — was reaching the model as an opening fragment plus a path it had to go fetch. The team context, decisions, and prior sessions you rely on now land in full at session start, and a stray warning on the way no longer makes it look like nothing loaded at all.
