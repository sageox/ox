...
- **A brief network drop no longer stops team-context sync for the rest of the day** — closing your laptop for four minutes was enough. Two pulls failed with "Could not resolve host", and a guard meant for a *permanently* broken checkout concluded the checkout was at fault and suspended background sync with no retry scheduled. Nothing unattended undid that: sync stayed dead for hours on a machine that was awake and online, and only `ox sync --all-teams` or a daemon restart brought it back. ox now looks at *what* failed — network and DNS errors take ordinary backoff and recover on their own, while a genuinely broken checkout is still suspended as before. The accompanying message also names a command that actually works, and a reclone now clears the suspension it invalidates.
- **A failed fetch no longer suppresses its own retry and reports success** — git empties `FETCH_HEAD` before it contacts the remote, so a fetch that failed left a fresh, empty file behind. ox read that as "someone just fetched, skip" — and reported the skip as a completed sync, clearing the failure and recording a sync time for work that never happened.
- **Team contexts are no longer re-cloned every hour, in every daemon** — ox asked git whether a checkout was a partial clone by reading a setting git 2.50 stopped writing, so the answer was always "no" and the hourly upgrade-to-partial pass re-cloned all of them, forever. Each reclone also discarded the checkout's local cache, which is how one daemon tells another when a team context last synced — so a perfectly healthy team context reported "not synced".
- **`ox status` no longer shows a green check for a team context that has not synced** — under a heading reading `Syncing`, the check meant only "this directory is a git repo". It reported everything healthy throughout a multi-hour outage that `ox daemon status` was reporting correctly the whole time.
- **`ox daemon status` stops reporting problems that are already resolved** — a completed automatic repair filed itself as a standing warning ("session meta titles: recovered=1"), and no code path could ever retire it, so it stayed for as long as the daemon ran. A repair is now an event; only checks that need a human, or that failed to run, remain listed.
- **The "last error" line clears once it stops being true** — a single DNS blip stayed on screen indefinitely, printed directly beside the "0 errors" that the same screen computed from the same list with a one-hour window.
- **Deleting a session no longer looks like a data-loss event in `ox daemon status`** — the background scan that reports plan/session deletions counted *files*, and one session is six or seven of them, so every ordinary delete read as a suspected wipe. On one real ledger, 32 of the 33 "mass deletions" it reported were nothing of the kind. It now counts whole plans and sessions actually removed, confirmed gone from the commit: deleting one or two is routine and stays quiet, tidying stray files inside a session registers as nothing at all, and a genuine mass deletion is still surfaced for review. The commit-time guard that *blocks* a wipe is unchanged.
- **`ox daemon status` reports how long the daemon has really been running** — uptime stopped counting while the machine slept, so it disagreed with the timestamps on its own warnings.
...
- **`ox doctor --fix` no longer deletes a healthy code index and reports it as a repair** — *any* failure of the integrity check counted as proof of corruption, including "I could not open this file." An index created under a different user, restored from a partial backup, or otherwise unreadable was removed — the entire index directory — and announced as "corrupt index removed." ox now separates "the check found damage" from "the check could not run": it deletes only on the first, reports the second and touches nothing, and retries a database busy with another process instead of mistaking it for damage. Genuinely corrupt indexes are still detected and rebuilt as before.
