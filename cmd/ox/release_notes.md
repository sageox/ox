...
- **A session still reaches your Ledger when its window closes without a clean stop** — if the client process died mid-recording, the transcript stayed on disk indefinitely: not uploaded, not repaired in the background, and not reported by `ox doctor`. Those sessions are now reclaimed and uploaded.
