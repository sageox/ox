...
- **Deleting a session no longer looks like a data-loss event** — the plan/session deletion guard counted *files*, and one session is six or seven of them, so ordinary cleanup tripped a threshold meant to catch a wipe of everything. It now counts whole plans and sessions actually removed: deleting one or two is routine and passes silently, tidying stray files inside a session registers as nothing at all, and a real mass deletion is still refused and surfaced.
