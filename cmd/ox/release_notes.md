...
Plans get a lifecycle you can track end-to-end with a genuinely better reviewing surface, and a session's link now resolves the moment it starts instead of when it ends.
...
- **`ox init` reliably stages everything it writes, and stops touching your root `.gitignore`.** A single bad file path used to silently break staging for the whole batch — hooks, skills, rules, instruction files, for any agent — so a fresh init could look complete while leaving new files untracked until you found and staged them yourself; that's fixed for every supported agent. Its own internal cache ignore rule also no longer gets written into your project's root `.gitignore`; already-affected repos clean up the next time you run `ox init` or `ox doctor --fix`.
