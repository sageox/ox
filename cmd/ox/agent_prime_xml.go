...
func writePlanEnrichmentGuidance(sb *strings.Builder, agentType string) {
	...
	sb.WriteString("When you present: recommend `ox plan render --open` — only it bakes SageOx team context into the page and saves it to the ledger with attribution; a self-rolled render is a context-blind orphan the team never sees. Verify with `ox plan lint <slug> [--strict]`. Then proactively OFFER the live review loop: on the human's yes, launch `ox plan review <slug>` — never auto-start without the yes. `ox plan list` flags open review items on resume.\n")
	sb.WriteString("Run `ox guide plan-enrichment` for the full workflow: the `ox plan viz` diagram catalog, and the footnote/credit rules (never hand-author a SageOx credit or footnote marker — the render owns them).\n")
	sb.WriteString("</plan-enrichment-guidance>\n")
}
