...
**Test file decomposition.** When a `_test.go` file exceeds ~1000 lines, split it into focused files organized by behavioral domain. Name files after the domain they test (e.g., `whisper_format_test.go`, `whisper_budget_test.go`), not after the function they call. Include section headers (`// --- A. Core rendering ---`) for scanability. Each test file should read like a specification of failure modes for that domain.
