...
**Implementation appendix — progressive disclosure at the DOCUMENT level (the biggest single cognitive-load lever).** A plan has two readers: the human approving in ten minutes and the agent that implements it. Mirror that split in the page. The **decision layer** — problem, approach, tradeoffs, biggest risk, hero diagram — IS the visible page. The **implementation layer** — exact `file:line` steps, code snippets, edit order, gotchas: everything the implementer needs and the approver skips — goes in ONE `<details>` **"Implementation notes"** block at the BOTTOM, collapsed by default and de-emphasized (dim, mono, smaller). The ten-minute reader never expands it; the implementer does. This is where the `file:line` steps live — never scattered through the decision layer up top. **Relocate detail here, don't delete it** (it is essential to the implementer) and don't inline it up top (it blows the reviewer's budget). If the plan has a scroll-spy TOC, mark the appendix entry distinctly (or omit it) so it never competes with the decision sections.
